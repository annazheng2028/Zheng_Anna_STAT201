...
index_spread <- which.max(win_spread)
max_spread <- win_spread[index_spread]
bears_score <- bears[index_spread]
opponent_score <- opponent[index_spread]
paste("The largest win spread is ", max_spread, " when the Bears scored ", bears_score, " points and the Opponent scored ", opponent_score, " points.")
```
...
```{r}
more_scores <- matrix(c(37, 9, 17, 17), ncol = 2, byrow = TRUE)
colnames(more_scores) <- c("bears", "opponent")
game_score <- rbind(game_score, more_scores)
print(game_score)
```
...
```{r}
win1_index <- which(game_score[,"win"] == TRUE)[1]
losses_pre_win1 <- sum(game_score[1:(win1_index - 1), "win"] == FALSE)
print(losses_pre_win1)
```
...
```{r}
area_rectangle <- function(length = 102, width = 60){
  area_sq_in <- length * width
  area_sq_ft <- (length/12)*(width/12)
  return(list(sq_in = area_sq_in, sq_ft = area_sq_ft))
}
result <- area_rectangle(length = 102, width = 60)
cat("The area in square inches is ", result$sq_in, ".\n", sep = "")
cat("The area in square feet is ", result$sq_ft, ".", sep = "")
```
...
longest_word <- find_longest_word(fruits)
print(longest_word)
```
...
value <- 2
check <- seq(1, 15, 3)
check_if_in_vector <- function(value, vector) {
  return(value %in% vector)
}
result <- check_if_in_vector(value,check)
```
...
```{r}
convert_negs_na <- function(input_matrix) {
...
```{r}
convert_even_odd_binary <- function(input_matrix) {
  input_matrix <- ifelse(input_matrix %% 2 == 0, 1, 0)
  return(input_matrix)
}
print("Original Matrix:")
print(x)
print("New Matrix:")
convert_even_odd_binary(x)
```
