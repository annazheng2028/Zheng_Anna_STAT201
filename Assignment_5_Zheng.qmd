...
Create a variable called `var_num` which is created by converting `var_string` to a numeric variable. 
...
output <- paste(var_seconds, "seconds is",
                min, ifelse(min == 1, "minute", "minutes"), "and",
                sec, ifelse(sec == 1, "second", "seconds"), ".")
print(output)
```
...
for (i in 1:var) {
  if ((i * i) > var) {
    cat(i, " squared is ", i*i, ", and is the first number whose square exceeds ", var, sep = "")
    break
  }
}
```